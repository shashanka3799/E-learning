...
import com.elearn.dbConnection.UploadTestDAO;
...
private String uploadFile(String path, Part filePart){
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			System.out.println("hi");
			String path = "F:/santhu2/eLAdmin/WebContent/tests";
			uploadFile(path, filePart);
			path = "F:/santhu2/eLearn/WebContent/tests";
		...
		upload.set_test_id   (testId);
		upload.set_subject   (subject);
		upload.set_date      (date);
		upload.set_file_path (filePath);
		
		System.out.println(upload.get_test_id());
		try {
			int res = UploadTestDAO.createTestInfo(upload);
